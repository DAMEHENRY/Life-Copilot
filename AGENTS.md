...
7. **远期客观锚点检索（默认执行）**：为每条主线主题生成具体检索句柄（名词、物品、地点、人名、场景词、精确短语，避免只搜抽象标签），按 memory → insights → archive → raw journal 顺序搜索。目标 `2-4` 个高质量锚点；持久主题至少包含 1 个 30 天以外的远期客观锚点（如能找到）。锚点必须是客观存在的记忆（objective anchor：旧日记、物品、地点、事件等有具体句柄的样本），不包括模糊情绪类比。读取选中锚点的原始日记全文，编织为微型时间线。若找不到远期锚点，明确说明缺口在哪一层。详见 `prompts/diary-mode.md` 的「远期客观锚点检索」和「历史编织规则」。
8. 若当天主题涉及实效性外部信息，可联网搜索作为辅助校验
9. 若命中文档里包含 `[[...]]` wikilink，继续跟读一层
10. 回复时必须明确区分：这是旧模式复现、旧假设验证、旧模式修正，还是新模式出现
