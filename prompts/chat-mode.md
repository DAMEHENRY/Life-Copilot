...
3. 先读 `journal/closing-greetings.md`，按其中规则拟好这句晚安，并在它的台账末尾记一行；再把它追加为当天 `## What Life Copilot Said` 的最后一段：读出该节现有内容，原样保留，末尾加上这句，写临时文件后用 `writeback-journal` 写回（该命令整节替换，不能只传这一句）。最后只输出恰好这一句晚安，不附执行报告。
