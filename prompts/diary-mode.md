...
3. 筛选锚点而非倾倒档案：目标 **2-4 个高质量锚点**；对于持久主题，至少包含 1 个 30 天以外的远期客观锚点（如能找到）。优先时间分散的锚点，而非多个近期重复条目。
4. 读取选中的原始日记全文，而非仅 rg 片段；按已有规则跟读 wikilink 一层。
5. 在最终分析中将锚点编织为微型时间线：旧模式/早期根源 → 近期延续 → 今天新确认、修正或复杂化了什么。
6. 若未找到好的远期锚点，明确说明缺口在热记忆、冷归档、insights 还是原始日记检索。
...
- 若历史证据充足，优先使用 `2-4` 个高质量历史锚点，而不是只给一个装饰性日期。对于持久主题，至少一个应为 30 天以外的远期客观锚点（如能找到）。
- 禁止只给模糊情绪类比（如「你以前也有类似感受」）；每个锚点必须是客观存在的记忆，有可检索的具体句柄。
- 回答中必须明确写出一句类似“旧模式是什么，这次新东西是什么”，避免只有回顾没有增量。
- 若历史证据不足，必须明确缺口主要在热记忆、冷归档、insights 索引还是原始样本日记检索。
...
**记忆更新：**
- 当 memory audit 结论不是 `无需写入` 时，先判断当天材料的自然记忆颗粒度：可能是 0 条、1 条，也可能是多条。
...
- 当用户明确要求“更新长期记忆”“直接更新”“不要问我”或类似表达时，主动写入所有值得沉淀的条目，不要为了确认每一条而中断。
- 若用户未明确授权写入，可在结尾用 1-2 句说明建议写入哪些条目；只有用户同意后，才调用 `writeback-memory`。
- 写入内容必须满足：一条一事实、可复用、可检索、避免纯情绪句。

**Tomorrow Projection：**
- 默认在回复结尾给出明天的最小动作建议；若执行写回，将建议单独写入目标日的 `## 🧭 Daily Suggestion`。
...
将分析正文（镜子、memory audit、历史锚点、微行动）写入临时 markdown 文件，然后执行：
```bash
python3 scripts/copilot.py writeback-journal --date YYYY-MM-DD --input-file <tmp-analysis-file>
```

**内容边界：**
- 写入内容只能是对当天日记的分析、镜子、memory audit。
- 不要包含工具日志、inbox audit 报告、执行报告。
- 不要伪装成 Henry 的日记正文。
...
**边界：**
- `What Life Copilot Said` 不承载次日建议；次日建议只进目标日日记的 `## 🧭 Daily Suggestion`。
...
**Daily Suggestion 语态：**
- 建议正文使用**目标日语态**——用 `今天` / `today`，不用 `明天` / `tomorrow`（除非确实指目标日之后的那天）。
...
3. 完成了远期客观锚点检索：为每条主线主题生成了具体检索句柄，按 memory → insights → archive → raw journal 顺序搜索，读取了选中锚点的原始日记全文。
4. 历史锚点数量：至少有 1 个 `[[YYYY-MM-DD]]` 历史锚点，或明确“证据不足”。当主题具有持久性时，分析应包含 `2-4` 个高质量历史锚点，且证据允许时至少一个应为 30 天以外的远期客观锚点。
5. 若找不到远期锚点，必须明确说明缺口在哪一层：热记忆（memory.md）、冷归档（memory-archive.md）、insights 索引（insights.jsonl）、还是原始日记检索（rg 搜索结果）。
6. 每个历史锚点必须是客观存在的记忆（有具体日期、场景、物品、事件），不允许模糊情绪类比。
7. 至少有 1 处镜子式增量洞察。
8. 至少有一句明确区分“旧模式 / 新变化 / 本次验证”。
9. 至少有 1 个 24 小时内可启动的微行动。
10. 结尾做了简短的 memory audit。
11. 主动使用了必要的 `[[文档名]]` 链接，且没有链接不存在的文件。
12. 无空泛鸡汤、无编造历史、无不必要的 bullet 化。
